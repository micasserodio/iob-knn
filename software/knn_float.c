...
float sq_dist( struct datum a, struct datum b) {
  float X = a.x-b.x;
  float X2=X*X;
  float Y = a.y-b.y;
  float Y2=Y*Y;
...
int main() {
  ...
  for (int i=0; i<N; i++) {
    ...
    //init coordinates
    data[i].x = (float) random_real(-100.0, 100.0);
    data[i].y = (float) random_real(-100.0, 100.0);
  ...
  //init test points
  for (int k=0; k<M; k++) {
    x[k].x  = (float) random_real(-100.0, 100.0);
    x[k].y  = (float) random_real(-100.0, 100.0);
